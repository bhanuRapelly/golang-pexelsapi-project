...
type Video struct {
	...
	VideoPictures []VideoFiles `json:"video_pictures"`
}
...
func (c *Client) SearchVideo(query, perPage, page int) (*VideoSearchResult, error) {
	url := fmt.Sprintf(VideoApi+"/search?query=%d&per_page=%d&page=%d", query, perPage, page)
	resp, err := c.requestDoWithAuth("GET", url)
	if err != nil {
		return nil, err
	}
...
func main() {
	...
	os.Setenv("PexelsToken", "563492ad6f91700001000001357dbf2922994e30a87c64a4db25e64c")
